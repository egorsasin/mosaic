import { AuthenticationMethod, DATA_SOURCE_PROVIDER, ExternalAuthenticationMethod, User } from '../../data';
...
export class ExternalAuthenticationService {
  ...
  async findUser(strategy: string, externalIdentifier: string): Promise<User | undefined> {
    ...
    const matchingUser = usersWithMatchingIdentifier.find((user) =>
      user.authenticationMethods.find(
        (method: AuthenticationMethod) => method instanceof ExternalAuthenticationMethod && method.strategy === strategy
      )
    );
  ...
  public async createUser(config: {
    ...
    const existingUser = await this.findExistingUserByIdentifier(config.emailAddress);
    ...
    const authMethod = await this.dataSource.getRepository(ExternalAuthenticationMethod).save(
  ...
  private async findExistingUserByIdentifier(identifier: string): Promise<User | undefined> {
