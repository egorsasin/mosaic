...
import { DATA_SOURCE_PROVIDER, MosaicEntity } from '../../../data';
...
export class EntityHydrator {
  constructor(
    @Inject(DATA_SOURCE_PROVIDER) private readonly connection: DataSource //private connection: TransactionalConnection, private productPriceApplicator: ProductPriceApplicator, //private translator: TranslatorService,
  ) {}
   ...
   * @since 1.3.0
   */
  async hydrate<Entity extends MosaicEntity>(
    ctx: RequestContext,
    target: Entity,
    options: HydrateOptions<Entity>
  ): Promise<Entity> {
    if (options.relations) {
      const missingRelations = this.getMissingRelations(target, options);
      ...
      if (missingRelations.length) {
        const hydratedQb: SelectQueryBuilder<MosaicEntity> = this.connection
          .getRepository<MosaicEntity>(target.constructor)
        ...
        for (const prop of propertiesToAdd) {
          target[prop] = mergeDeep(target[prop], hydrated[prop]);
        }
  ...
  private assignSettableProperties<Entity extends MosaicEntity>(
    target: Entity,
    source: Entity
  ) {
   ...
   */
  private getMissingRelations<Entity extends MosaicEntity>(
    target: Entity,
    options: HydrateOptions<Entity>
  ) {
    ...
    return unique(
      missingRelations.filter((relation) => !relation.endsWith('.customFields'))
    );
  }

  //   private getRequiredProductVariantRelations<Entity extends MosaicEntity>(
  //     target: Entity,
  ...
  private getRelationEntityAtPath(
    entity: MosaicEntity,
    path: string[]
  ): MosaicEntity | MosaicEntity[] | undefined {
    let isArrayResult = false;
    const result: MosaicEntity[] = [];
  ...
  private getRelationEntityTypeAtPath(
    ...
    const { entityMetadatas } = this.connection;
    ...
    return currentMetadata.target as Type<MosaicEntity>;
  }

  //   private isTranslatable<T extends MosaicEntity>(
  //     input: T | T[] | undefined
  //   ): boolean {
