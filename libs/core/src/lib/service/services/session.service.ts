...
export class SessionService {
  ...
  public async deleteSessionsByActiveOrderId(
  ...
  async deleteSessionsByUser(ctx: RequestContext, user: User): Promise<void> {
    const userSessions = await this.dataSource
  ...
  async createAnonymousSession(): Promise<CachedSession> {
    ...
    const newSession = await this.dataSource
  ...
  private withTimeout<T>(maybeSlow: Promise<T> | T): Promise<T | undefined> {
    return Promise.race([
      new Promise<undefined>((resolve) =>
        setTimeout(() => resolve(undefined), SESSION_CACHE_TIMEOUT_MS)
      ),
      maybeSlow,
    ]);
  }
  ...
  private isAuthenticatedSession(
    ...
    return Object.prototype.hasOwnProperty.call(session, 'user');
  }
}
