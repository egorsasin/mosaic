...
export class SelectionManager<T> {
  ...
  public selectionChanges$: Observable<T[]>;

  private selectionInternal: T[] = [];
  private items: T[] = [];
  private selectionChangesSubject = new Subject<T[]>();

  publicsetMultiSelect(isMultiSelect: boolean) {
  ...
  public toggleSelection(item: T, event?: MouseEvent) {
